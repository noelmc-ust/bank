...
public class CreditCardServiceImpl implements CreditCardService {
    ...
    public DataResult<CreditCardDTO> findById(Long creditCardId) {
        ...
        // ✅ fix the message key used in log
        logProducer.sendToLog(prepareLogRequest(OperationType.GET, Messages.CREDIT_CARD_FOUND));
    ...
    // ✅ New for customer "My Cards"
    @Cacheable(value = Caches.CREDIT_CARDS_CACHE, key = "'user:' + #userId", unless = "#result.success != true")
    @Override
    public DataResult<List<CreditCardDTO>> findByUserId(Long userId) {
        List<CreditCard> list = creditCardRepository.findAllByUserId(userId);

        logProducer.sendToLog(prepareLogRequest(OperationType.GET, Messages.CREDIT_CARDS_LISTED));

        return new SuccessDataResult<>(
                CreditCardMapper.INSTANCE.convertCreditCardDTOs(list),
                Messages.CREDIT_CARDS_LISTED
        );
    }
    ...
    public DataResult<UserResponse> findCreditCardUserByUserId(Long userId) {
        ...
        return new SuccessDataResult<>(userResponse, Messages.CREDIT_CARD_USER_FOUND);
    }
    ...
    public DataResult<BankResponse> findCreditCardBankByBankId(Long bankId) {
        ...
        return new SuccessDataResult<>(bankResponse, Messages.CREDIT_CARD_BANK_FOUND);
    }
